...
def test_send_detection():
    ...
    # Resolve directories robustly, independent of current working directory
    script_dir = Path(__file__).resolve().parent
    repo_root = script_dir.parents[2]

    # Check if we have a crop to test with (next to this script)
    crops_dir = script_dir / ".trash_crops"
    if not crops_dir.exists() or not list(crops_dir.glob("*.png")):
        print("❌ No crops found in .trash_crops/")
        print("   Run: python trash_analyzer.py bottlewater.jpeg --debug")
        return False
    ...
    # Copy to public directory (resolved from repo root)
