...
@app.get("/detection-counts")
def get_detection_counts():
    """
    Get detection counts from the most recent video processing.
    Returns the counts JSON file generated by yolov8_seg_track.py
    """
    counts_file = OUTPUTS_DIR / "output.counts.json"
    if not counts_file.exists():
        return jsonify({"counts": {}, "available": False})
    
    try:
        with open(counts_file, 'r') as f:
            counts = json.load(f)
        return jsonify({"counts": counts, "available": True})
    except Exception as e:
        return jsonify({"counts": {}, "available": False, "error": str(e)})
