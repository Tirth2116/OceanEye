...
// ============================================
// EDIT THIS ARRAY TO CHANGE THE TEST CASE DATA
// ============================================
// Format: Array of objects with id, x, y, and type
// x and y coordinates should be between 0-100
// Available types: "rov", "trash_plastic", "trash_metal", "trash_paper", 
// "trash_fabric", "trash_wood", "trash_fishing_gear", "trash_rubber", 
// "trash_etc", "animal_fish", "animal_starfish", "animal_shells", 
// "animal_crab", "animal_eel", "animal_etc", "plant"
...
// ============================================
// EDIT THIS DICTIONARY TO CHANGE THE TEST CASE DATA (COUNT FORMAT)
// ============================================
// Format: Object mapping trash types to counts
// Positions will be randomly generated deterministically based on the counts
...
export default function VisualizerPage() {
  // Using DEFAULT_COUNT_SAMPLE (dictionary format) instead of DEFAULT_INPUT (array format)
  // Change this to DEFAULT_INPUT if you want to use explicit coordinates
    ...
    <div className="min-h-[calc(100vh-4rem)] px-4 py-10">
      <div className="max-w-[1600px] mx-auto space-y-10">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_330px]">
          ...
          <aside className="space-y-6 relative" style={{
            background: `
              linear-gradient(rgba(255,255,255,0.04) 1px, transparent 1px),
              linear-gradient(90deg, rgba(255,255,255,0.04) 1px, transparent 1px),
              linear-gradient(to bottom, rgba(7, 30, 64, 0.95), rgba(6, 59, 102, 0.95))
            `,
            backgroundSize: '80px 80px, 80px 80px, 100% 100%',
          }}>
            <div 
              className="border border-white/10 bg-white/5 p-6 shadow-xl backdrop-blur relative z-10 overflow-hidden"
              style={{
                borderRadius: '1.5rem',
                ...
                <ul className="mt-4 space-y-2">
                  {plan.uniqueTypes.map((type) => (
                    <li
                      key={type}
                      className={cn(
                        "flex items-center justify-between px-4 py-2.5 text-sm text-slate-100",
                        "rounded-[1.5rem] overflow-hidden",
                        "bg-white/5 backdrop-blur-sm",
                        type === "rov" 
                          ? "ring-1 ring-cyan-400/60 bg-cyan-400/10 text-white" 
                    ...
                    >
                      <span className="flex items-center gap-3 font-medium">
                        <span className="text-xl leading-none">{TYPE_ICONS[type]}</span>
                        <span className="leading-tight">{humanizeLabel(type)}</span>
                      </span>
                      <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-medium text-slate-200 shrink-0 ml-2">
                        {
...
function normalizePoints(points: TrashPoint[], width: number, height: number): NormalizedTrashPoint[] {
  ...
  // Use 8% padding on all sides for visual breathing room
  const padding = Math.min(width, height) * 0.08
  const drawableWidth = width - padding * 2
  const drawableHeight = height - padding * 2

  // Scale to fill available space while maintaining aspect ratio
  const scaleX = drawableWidth / rangeX
  const scaleY = drawableHeight / rangeY
